...
namespace Nes
{
    namespace Core
    {
        ...
        bool Cartridge::Unif::NewChunk(byte& processed,const dword id)
        {
            ...
            {
                char name[5];
                Log() << "Unif: warning, duplicate chunk: \"" << ChunkName(name,id) << "\" ignored" "\n";
                return false;
            }
        }
        ...
        void Cartridge::Unif::ReadHeader()
        {
            ...
            Log() << "Unif: revision " << stream.Read32() << "\n";
            ...
            for (uint i=0; i < HEADER_RESERVED_LENGTH; ++i)
            {
                ...
                if (reserved[i])
                {
                    Log() << "Unif: warning, unknown header data" "\n";
                    break;
                }
            }
        }
        ...
        dword Cartridge::Unif::ReadString(cstring const logtext,Vector<char>* string=__null)
        {
            Vector<char> tmp;
        ...
        dword Cartridge::Unif::ReadDumper()
        {
            ...
            if (*dump.name)
                log << "Unif: dumped by: " << dump.name << "\n";
            
            log << "Unif: dump year: " << dump.year << "\n"
            "Unif: dump month: " << dump.month << "\n"
            "Unif: dump day: " << dump.day << "\n";
            
            if (*dump.agent)
                log << "Unif: dumper agent: " << dump.agent << "\n";
        ...
        dword Cartridge::Unif::ReadSystem()
        {
            switch (stream.Read8())
            {
                ...
                case 1:
                    ...
                    Log::Flush( "Unif: PAL system" "\n" );
                    break;
                    
                default:
                    
                    info.setup.region = Ref::REGION_BOTH;
                    Log::Flush( "Unif: NTSC/PAL system" "\n" );
                    break;
            }
        ...
        {
            (__builtin_expect(!(!!(type < 2 && index < 16)), 0) ? __assert_rtn(__func__, "/Users/Jorrit/iOS/nestopia/core/NstCartridgeUnif.cpp", 531, "!!(type < 2 && index < 16)") : (void)0);
        ...
        {
            (__builtin_expect(!(!!(type < 2 && index < 16)), 0) ? __assert_rtn(__func__, "/Users/Jorrit/iOS/nestopia/core/NstCartridgeUnif.cpp", 547, "!!(type < 2 && index < 16)") : (void)0);
        ...
        dword Cartridge::Unif::ReadBattery()
        {
            ...
            Log::Flush( "Unif: battery present" "\n" );
            return 0;
        }
        ...
        dword Cartridge::Unif::ReadMirroring()
        {
            switch (stream.Read8())
            {
                case 0: info.setup.mirroring = Ref::MIRROR_HORIZONTAL; Log::Flush( "Unif: horizontal mirroring" "\n" ); break;
                case 1: info.setup.mirroring = Ref::MIRROR_VERTICAL; Log::Flush( "Unif: vertical mirroring" "\n" ); break;
                case 2: info.setup.mirroring = Ref::MIRROR_ZERO; Log::Flush( "Unif: zero mirroring" "\n" ); break;
                case 3: info.setup.mirroring = Ref::MIRROR_ONE; Log::Flush( "Unif: one mirroring" "\n" ); break;
                case 4: info.setup.mirroring = Ref::MIRROR_FOURSCREEN; Log::Flush( "Unif: four-screen mirroring" "\n" ); break;
                default: info.setup.mirroring = Ref::MIRROR_CONTROLLED; Log::Flush( "Unif: mapper controlled mirroring" "\n" ); break;
            }
        ...
        dword Cartridge::Unif::ReadController()
        {
            ...
            {
                log << ((controller & (0x40|0x80)) ? "unknown" "\n" : "unspecified" "\n");
            }
        ...
        {
            Log::Flush( "Unif: CHR is writable" "\n" );
            return 0;
        }
        ...
        {
            char name[5];
            Log() << "Unif: warning, skipping unknown chunk: \"" << ChunkName(name,id) << "\"" "\n";
            ((void)0);
            return 0;
        }
