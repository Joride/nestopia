...
namespace Nes
{
    namespace Core
    {
        namespace Boards
        {
            ...
            Data Unl8157::Peek_Prg(void* p_,Address i_) { return static_cast<Unl8157*>(p_)->Peek_M_Prg(i_); } inline Data Unl8157::Peek_M_Prg(Address address)
            {
                return !trash ? prg.Peek( address - 0x8000 ) : 0xFF;
            }
            
            void Unl8157::Poke_Prg(void* p_,Address i_,Data j_) { static_cast<Unl8157*>(p_)->Poke_M_Prg(i_,j_); } inline void Unl8157::Poke_M_Prg(Address address,Data)
            {
