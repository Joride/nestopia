...
namespace Nes
{
    namespace Core
    {
        namespace Boards
        {
                ...
                if (baseChunk == AsciiId<'M','M','1'>::V)
                {
                    while (const dword chunk = state.Begin())
                    {
                        if (chunk == AsciiId<'R','E','G'>::V)
                        {
                            ...
                            serial.shifter = ((data[5]) < (5) ? (data[5]) : (5));
                        }
            ...
            {
                (__builtin_expect(!(!!(index < 4)), 0) ? __assert_rtn(__func__, "/Users/Jorrit/iOS/nestopia/core/board/NstBrdMmc1.cpp", 199, "!!(index < 4)") : (void)0);
            ...
            void Mmc1::Poke_Prg(void* p_,Address i_,Data j_) { static_cast<Mmc1*>(p_)->Poke_M_Prg(i_,j_); } inline void Mmc1::Poke_M_Prg(Address address,Data data)
            {
