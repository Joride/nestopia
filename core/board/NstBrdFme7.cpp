    ...
    namespace Core
    {
        namespace Boards
        {
            ...
            private:
                ...
                inline Data Peek_M_4017(Address); static Data Peek_4017(void*,Address);
                inline void Poke_M_4017(Address,Data); static void Poke_4017(void*,Address,Data);
            ...
            irq (c.cpu),
            sound (c.apu),
            barcodeWorld (c.attribute == ATR_BARCODE_READER ? new BarcodeWorld : __null)
            {}
            ...
            bool Fme7::BarcodeWorld::SubTransfer(cstring const string,const uint length,byte* __restrict__ output)
            {
                typedef char Nestopia_assertion_at_line_462[(MAX_DATA_LENGTH >= 191) ? 1 : -1];
            ...
            void Fme7::BarcodeWorld::Poke_4017(void* p_,Address i_,Data j_) { static_cast<Fme7::BarcodeWorld*>(p_)->Poke_M_4017(i_,j_); } inline void Fme7::BarcodeWorld::Poke_M_4017(Address address,Data data)
            {
                p4017.Poke( address, data );
            }
            
            Data Fme7::BarcodeWorld::Peek_4017(void* p_,Address i_) { return static_cast<Fme7::BarcodeWorld*>(p_)->Peek_M_4017(i_); } inline Data Fme7::BarcodeWorld::Peek_M_4017(Address address)
            {
                return (IsTransferring() ? Fetch() : 0x00) | p4017.Peek( address );
            }
            
            void Fme7::Poke_8000(void* p_,Address i_,Data j_) { static_cast<Fme7*>(p_)->Poke_M_8000(i_,j_); } inline void Fme7::Poke_M_8000(Address,Data data)
            {
                command = data;
            }
            
            void Fme7::Poke_A000(void* p_,Address i_,Data j_) { static_cast<Fme7*>(p_)->Poke_M_A000(i_,j_); } inline void Fme7::Poke_M_A000(Address,Data data)
            {
            ...
            void Fme7::Poke_C000(void* p_,Address i_,Data j_) { static_cast<Fme7*>(p_)->Poke_M_C000(i_,j_); } inline void Fme7::Poke_M_C000(Address,Data data)
            {
            ...
            void Fme7::Sound::Square::UpdateFrequency(const uint fixed)
            {
                ...
                frequency = ((length) < (1) ? (1) : (length)) * 16 * fixed;
                timer = ((timer + idword(frequency) - prev) < (0) ? (0) : (timer + idword(frequency) - prev));
            }
            ...
            {
                const idword prev = frequency;
                frequency = ((length*16) < (1*8) ? (1*8) : (length*16)) * fixed;
                timer = ((timer + idword(frequency) - prev) < (0) ? (0) : (timer + idword(frequency) - prev));
            }
            ...
            void Fme7::Poke_E000(void* p_,Address i_,Data j_) { static_cast<Fme7*>(p_)->Poke_M_E000(i_,j_); } inline void Fme7::Poke_M_E000(Address,Data data)
            {
                sound.WriteReg( data );
            }
            ...
            inline dword Fme7::Sound::Envelope::Clock(const Cycle rate)
            {
                if (!holding)
                {
                    timer -= idword(rate);
            ...
            inline dword Fme7::Sound::Noise::Clock(const Cycle rate)
            {
            ...
            inline dword Fme7::Sound::Square::GetSample(const Cycle rate,const uint envelope,const uint noise)
            {
                dword sum = timer;
                timer -= idword(rate);
                ...
                if ((noise|status) & 0x8 && out)
                {
                    ...
                    else
                    {
                        ...
                        {
                            dc ^= (status & 0x1) - 1UL;
                            sum += ((dword(-timer)) < (frequency) ? (dword(-timer)) : (frequency)) & dc;
                            timer += idword(frequency);
                        }
                        while (timer < 0);
