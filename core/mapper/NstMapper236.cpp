...
namespace Nes
{
    namespace Core
    {
        ...
        {
            typedef char Nestopia_assertion_at_line_85[(ATR_GENERIC == 0 && ATR_800_IN_1 == 1 && ATR_76_IN_1 == 2) ? 1 : -1];
        ...
        Data Mapper236::Peek_Prg(void* p_,Address i_) { return static_cast<Mapper236*>(p_)->Peek_M_Prg(i_); } inline Data Mapper236::Peek_M_Prg(Address address)
        {
            return prg.Peek( mode ? (address & 0x7FF0) | cartSwitches.GetMode() : address - 0x8000 );
        }
        
        void Mapper236::Poke_Prg(void* p_,Address i_,Data j_) { static_cast<Mapper236*>(p_)->Poke_M_Prg(i_,j_); } inline void Mapper236::Poke_M_Prg(Address address,Data)
        {
            uint banks[2] =
            {
