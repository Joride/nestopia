...
namespace Nes
{
    namespace Core
    {
        ...
        databaseHandle (h)
        {
            typedef char Nestopia_assertion_at_line_75[(Ref::PPU_RP2C03B == 1 && Ref::PPU_RP2C03G == 2 && Ref::PPU_RP2C04_0001 == 3 && Ref::PPU_RP2C04_0002 == 4 && Ref::PPU_RP2C04_0003 == 5 && Ref::PPU_RP2C04_0004 == 6 && Ref::PPU_RC2C03B == 7 && Ref::PPU_RC2C03C == 8 && Ref::PPU_RC2C05_01 == 9 && Ref::PPU_RC2C05_02 == 10 && Ref::PPU_RC2C05_03 == 11 && Ref::PPU_RC2C05_04 == 12 && Ref::PPU_RC2C05_05 == 13) ? 1 : -1];
            result = Collect();
        ...
        Result Cartridge::Ines::ReadHeader(Ref::Setup& setup,const byte* const file,const ulong length)
        {
            ...
            else
            {
                ...
                setup.wrkRamBacked = ((header[6] & 0x2U) ? ((header[8]) < (1U) ? (1U) : (header[8])) * dword(SIZE_8K) : 0);
        ...
        dword Cartridge::Ines::Process()
        {
            ...
            if (info.setup.version)
                log << title << "version 2.0 detected" "\n";
            
            if (result == RESULT_WARN_BAD_FILE_HEADER)
                log << title << "warning, unknown or invalid header data!" "\n";
            ...
            if (info.setup.wrkRamBacked)
            {
                ...
                << (info.setup.wrkRamBacked >= SIZE_1K ? " battery-backed W-RAM set" "\n" : " non-volatile W-RAM set" "\n");
            }
            ...
            if (info.setup.system == Ref::SYSTEM_VS)
            {
                ...
                if (info.setup.version)
                {
                    if (info.setup.ppu)
                    {
                        ...
                        (__builtin_expect(!(!!(info.setup.ppu < 1+sizeof(array(names)))), 0) ? __assert_rtn(__func__, "/Users/Jorrit/iOS/nestopia/core/NstCartridgeInes.cpp", 492, "!!(info.setup.ppu < 1+sizeof(array(names)))") : (void)0);
                        log << title << names[info.setup.ppu-1] << " PPU set" "\n";
                    }
                    ...
                    if (info.setup.security)
                    {
                        ...
                        (__builtin_expect(!(!!(info.setup.security < 1+sizeof(array(names)))), 0) ? __assert_rtn(__func__, "/Users/Jorrit/iOS/nestopia/core/NstCartridgeInes.cpp", 505, "!!(info.setup.security < 1+sizeof(array(names)))") : (void)0);
                        log << title << names[info.setup.security-1] << " VS mode set" "\n";
                    }
                }
            }
            else if (info.setup.system == Ref::SYSTEM_PC10)
            {
                log << title << "Playchoice 10 set" "\n";
            }
            ...
            if (info.setup.version && info.setup.subMapper)
                log << title << "unknown sub-mapper " << info.setup.subMapper << " set" "\n";
            
            if (info.setup.trainer)
                log << title << "trainer set" "\n";
            
            databaseHandle = __null;
            ...
            if (info.setup.trainer && info.setup.wrkRamBacked+info.setup.wrkRam < SIZE_8K)
            {
                ...
                log << title << "warning, forcing 8k of W-RAM for trainer" "\n";
            }
        ...
        {
            ImageDatabase::Handle handle = __null;
            ...
            if (stream && maxlength > 16+MIN_CRC_SEARCH_STRIDE)
            {
                ...
                if (handle == __null && maxlength > length)
        ...
        {
            (__builtin_expect(!(!!(database)), 0) ? __assert_rtn(__func__, "/Users/Jorrit/iOS/nestopia/core/NstCartridgeInes.cpp", 579, "!!(database)") : (void)0);
            ...
            if (info.setup.wrkRam != database->WrkRam(handle))
            {
                ...
                << (info.setup.wrkRam % SIZE_1K ? info.setup.wrkRam : info.setup.wrkRam / SIZE_1K)
                << (info.setup.wrkRam % SIZE_1K ? " bytes" "\n" : "k" "\n");
            }
            ...
            if (info.setup.wrkRamBacked != database->WrkRamBacked(handle))
            {
                ...
                << (info.setup.wrkRamBacked % SIZE_1K ? info.setup.wrkRamBacked : info.setup.wrkRamBacked / SIZE_1K)
                << (info.setup.wrkRamBacked % SIZE_1K ? " bytes" "\n" : "k" "\n");
            }
            ...
            if (info.setup.mirroring != database->GetMirroring(handle))
            {
                ...
                typedef char Nestopia_assertion_at_line_743[(Ref::MIRROR_HORIZONTAL < 6 && Ref::MIRROR_VERTICAL < 6 && Ref::MIRROR_FOURSCREEN < 6 && Ref::MIRROR_ZERO < 6 && Ref::MIRROR_ONE < 6 && Ref::MIRROR_CONTROLLED < 6) ? 1 : -1];
                cstring types[6];
            ...
            if (info.setup.trainer != database->Trainer(handle))
            {
                ...
                log << title << (info.setup.trainer ? "enabled trainer" "\n" : "ignored trainer" "\n");
            }
