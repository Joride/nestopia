...
namespace Nes
{
    namespace Core
    {
        ...
        public:
            ...
            {
                (__builtin_expect(!(!!(i < NUM_FRAMES)), 0) ? __assert_rtn(__func__, "/Users/Jorrit/iOS/nestopia/core/NstTrackerRewinder.cpp", 82, "!!(i < NUM_FRAMES)") : (void)0);
        ...
        public:
            ...
            Mutex()
            {
                ...
                Output::lockCallback.Set( __null, __null );
                Output::unlockCallback.Set( __null, __null );
            }
        ...
        bool Tracker::Rewinder::ReverseSound::Update()
        {
            ...
            (__builtin_expect(!(!!(total)), 0) ? __assert_rtn(__func__, "/Users/Jorrit/iOS/nestopia/core/NstTrackerRewinder.cpp", 279, "!!(total)") : (void)0);
        ...
        bool Tracker::Rewinder::Key::Input::EndForward()
        {
            if (pos == 0)
            {
                ...
                if (Zlib::AVAILABLE && pos >= MIN_COMPRESSION_SIZE)
                {
                    ...
                    {
                        (__builtin_expect(!(!!(size < pos)), 0) ? __assert_rtn(__func__, "/Users/Jorrit/iOS/nestopia/core/NstTrackerRewinder.cpp", 338, "!!(size < pos)") : (void)0);
                        tmp.SetTo( size );
        ...
        {
            (__builtin_expect(!(!!(!saveState || !loadState)), 0) ? __assert_rtn(__func__, "/Users/Jorrit/iOS/nestopia/core/NstTrackerRewinder.cpp", 441, "!!(!saveState || !loadState)") : (void)0);
        ...
        {
            (__builtin_expect(!(!!(frame < NUM_FRAMES && (pingpong == 1U-0U || pingpong == 0U-1U))), 0) ? __assert_rtn(__func__, "/Users/Jorrit/iOS/nestopia/core/NstTrackerRewinder.cpp", 526, "!!(frame < NUM_FRAMES && (pingpong == 1U-0U || pingpong == 0U-1U))") : (void)0);
        ...
        template<typename T>
        inline Sound::Output* Tracker::Rewinder::ReverseSound::StoreType()
        {
            (__builtin_expect(!(!!(index <= NUM_FRAMES+LAST_FRAME)), 0) ? __assert_rtn(__func__, "/Users/Jorrit/iOS/nestopia/core/NstTrackerRewinder.cpp", 546, "!!(index <= NUM_FRAMES+LAST_FRAME)") : (void)0);
        ...
        {
            typedef char Nestopia_assertion_at_line_588[(NUM_FRAMES % 2 == 0) ? 1 : -1];
        ...
        {
            const T* __restrict__ src = static_cast<const T*>(input);
            
            for (uint i=0; i < 2; ++i)
            {
                if (const dword length = (target.length[i] << stereo))
                {
                    T* __restrict__ dst = static_cast<T*>(target.samples[i]);
        ...
        void Tracker::Rewinder::Execute(Video::Output* videoOut,Sound::Output* soundOut,Input::Controllers* inputOut)
        {
            try
            {
                ...
                (__builtin_expect(!(!!(frame < NUM_FRAMES)), 0) ? __assert_rtn(__func__, "/Users/Jorrit/iOS/nestopia/core/NstTrackerRewinder.cpp", 656, "!!(frame < NUM_FRAMES)") : (void)0);
                ...
                if (!rewinding)
                {
                    if (++frame == NUM_FRAMES)
                    {
                        ...
                        key->EndForward();
                        key = NextKey();
                        key->BeginForward( emulator, emuSaveState, __null );
                    }
                }
                else
                {
                    if (++frame == NUM_FRAMES)
                    {
                        ...
                        else
                        {
                            ...
                            key->Invalidate();
                            key = NextKey();
                            key->BeginForward( emulator, __null, emuLoadState );
        ...
        void Tracker::Rewinder::ChangeDirection()
        {
            ...
            if (rewinding)
            {
                for (uint i=frame; i < LAST_FRAME; ++i)
                    (emulator.*emuExecute)( __null, __null, __null );
                ...
                
                {
                    ...
                    {
                        video.Store();
                        (emulator.*emuExecute)( __null, sound.Store(), __null );
                    }
                }
        ...
        Data Tracker::Rewinder::Peek_Port_Put(void* p_,Address i_) { return static_cast<Tracker::Rewinder*>(p_)->Peek_M_Port_Put(i_); } inline Data Tracker::Rewinder::Peek_M_Port_Put(Address address)
        {
            return key->Put( ports[address-0x4016]->Peek( address ) );
        }
        
        Data Tracker::Rewinder::Peek_Port_Get(void* p_,Address i_) { return static_cast<Tracker::Rewinder*>(p_)->Peek_M_Port_Get(i_); } inline Data Tracker::Rewinder::Peek_M_Port_Get(Address)
        {
            return key->Get();
        }
        
        void Tracker::Rewinder::Poke_Port(void* p_,Address i_,Data j_) { static_cast<Tracker::Rewinder*>(p_)->Poke_M_Port(i_,j_); } inline void Tracker::Rewinder::Poke_M_Port(Address address,Data data)
        {
