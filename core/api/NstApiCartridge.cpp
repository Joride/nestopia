...
namespace Nes
{
    namespace Api
    {
        ...
        Result Cartridge::ReadNesHeader(Setup& setup,const void* const data,const ulong length) throw()
        {
            return Core::Cartridge::Ines::ReadHeader( setup, static_cast<const byte*>(data), length );
        }
        
        Result Cartridge::WriteNesHeader(const Setup& setup,void* data,ulong length) throw()
        {
        ...
        {
            if (emulator.imageDatabase == __null)
        ...
        {
            return emulator.imageDatabase ? emulator.imageDatabase->Search( crc ) : __null;
        }
        
        Cartridge::Database::Entry Cartridge::Database::FindEntry(const void* file,ulong length) const throw()
        {
            return emulator.imageDatabase ? Core::Cartridge::Ines::SearchDatabase( *emulator.imageDatabase, static_cast<const byte*>(file), length ) : __null;
        }
