...
namespace Nes
{
    namespace Core
    {
        namespace Peripherals
        {
            ...
            {
                typedef char Nestopia_assertion_at_line_51[(Region::NTSC == 0 && Region::PAL == 1) ? 1 : -1];
            }
            ...
            void DataRecorder::Hook_Tape(void* p_) { static_cast<DataRecorder*>(p_)->Hook_M_Tape(); } inline void DataRecorder::Hook_M_Tape()
            {
                for (const Cycle next = cpu.GetCycles() * clocks[cpu.GetRegion()][0]; cycles < next; cycles += clocks[cpu.GetRegion()][1])
                {
                    ...
                    {
                        (__builtin_expect(!(!!(status == RECORDING)), 0) ? __assert_rtn(__func__, "/Users/Jorrit/iOS/nestopia/core/NstPrpDataRecorder.cpp", 309, "!!(status == RECORDING)") : (void)0);
            ...
            void DataRecorder::Poke_4016(void* p_,Address i_,Data j_) { static_cast<DataRecorder*>(p_)->Poke_M_4016(i_,j_); } inline void DataRecorder::Poke_M_4016(Address address,Data data)
            {
                out = data;
                p4016->Poke( address, data );
            }
            
            Data DataRecorder::Peek_4016(void* p_,Address i_) { return static_cast<DataRecorder*>(p_)->Peek_M_4016(i_); } inline Data DataRecorder::Peek_M_4016(Address address)
            {
